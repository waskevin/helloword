...
官方文档覆盖面较广，但目前更像“规范片段的集合”，缺少一条新手可以照着走通的闭环路径。本文只保留能够归因于官方文档表达、结构或流程缺失的问题；纯粹的环境问题、操作失误和 Debian 基础错误不纳入。
...
### P2-12：升级、卸载、权限和数据目录虽有说明，但缺少可观察验收结果
...
以下问题在真实开发中确实出现过，但主要属于开发者自身操作或测试环境问题，因此不作为“官方文档问题”提交：

- 维护脚本权限为 `640`，而不是可执行权限；这是 Debian 包的基础打包错误。
- 使用 Windows 工具生成 `webui.bz2` 失败；官方已提供 Ubuntu 虚拟机/容器方向，具体工具行为属于本地环境差异。
- 8080 端口被 Apache 或 Docker 占用；这是测试设备环境问题，文档最多只能加强提醒。
- 远程 Shell 引号导致 JSON 没有更新；这是远程命令编写错误，不是 TOS 规范歧义。
- 服务刚启动时立即访问得到连接失败；这是启动时序问题，应通过等待和重试处理。
